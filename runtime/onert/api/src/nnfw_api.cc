...
{
  NNFW_RETURN_ERROR_IF_NULL(multi_sess);

  *multi_sess = new (std::nothrow) multi_session();
  if (*multi_sess == nullptr)
    return NNFW_STATUS_OUT_OF_MEMORY;
  return NNFW_STATUS_NO_ERROR;
}
...
{
  NNFW_RETURN_ERROR_IF_NULL(multi_sess);
  return multi_sess->add_to_multisession(package_file_path, backends);
}
...
{
  NNFW_RETURN_ERROR_IF_NULL(multi_sess);
  return multi_sess->multisession_set_input(session_num, index, type, buffer, length);
}
...
{
  NNFW_RETURN_ERROR_IF_NULL(multi_sess);
  return multi_sess->multisession_set_output(session_num, index, type, buffer, length);
}

NNFW_STATUS nnfw_multisession_run_sync(multi_session *multi_sess, int session_num)
{
  NNFW_RETURN_ERROR_IF_NULL(multi_sess);
  return multi_sess->multisession_run_sync(session_num);
}

NNFW_STATUS nnfw_multisession_run_async(multi_session *multi_sess)
{
  NNFW_RETURN_ERROR_IF_NULL(multi_sess);
  return multi_sess->multisession_run_async();
}
...
{
  NNFW_RETURN_ERROR_IF_NULL(multi_sess);
  return multi_sess->multisession_set_async_input(session_num, index, type, buffer, length);
}

NNFW_STATUS nnfw_multisession_get_result(multi_session *multi_sess, int session_num, std::vector<void *> outputs)
{
  NNFW_RETURN_ERROR_IF_NULL(multi_sess);
  return multi_sess->multisession_get_result(session_num, outputs);
}

NNFW_STATUS nnfw_multisession_set_finish(multi_session *multi_sess, int session_num)
{
  NNFW_RETURN_ERROR_IF_NULL(multi_sess);
  return multi_sess->multisession_set_finish(session_num);
}

NNFW_STATUS nnfw_multisession_wait(multi_session *multi_sess, int session_num)
{
  NNFW_RETURN_ERROR_IF_NULL(multi_sess);
  return multi_sess->multisession_wait(session_num);
}
